	...
	managerStatus:
	{
		addRow:
		{
			activateWirelessNetwork: function(index)
			{
				...
				activateWirelessNetworkSpanTemplate = '<span ml="__ML_wireless_network_lan"></span> <instance_index> <span id="<id>" ml="__ML_wireless_network_enabled"></span>'.replace("<instance_index>", (index+1) + " (" + selectedFreqName + ")").replace("<id>", getId.networkEnabledLabelId());
			...
			connectedClients: function(index)
			{
				...
				for(var i=0; i<instance.aps.length; i++)
				{
					...
					var ssidSpanValueTemplate = '<span id="<id>"><text></span>'.replace("<text>", instance.aps[i].ssid);
